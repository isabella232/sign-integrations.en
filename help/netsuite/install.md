---
title: Adobe Sign for NetSuite - Installation and Customization Guide (v4.0.4)
description: Adobe Sign for NetSuite - Installation and Customization Guide 
product: Adobe Sign
...
Adobe Sign for NetSuite provides a complete eSignature integration with NetSuite. You can use Adobe Sign for Netsuite integration to send agreements such as contracts, quotes, and other documents, which require electronic signatures, to recipients directly from NetSuite. You can create and send Adobe Sign agreements from customer, lead, quote, and other NetSuite records. Adobe Sign updates NetSuite with the current status of agreements and stores the agreements with the associated NetSuite records once they are fully executed. You can view the history of all agreements sent from NetSuite from within the product.

Refer to the [Adobe Sign for NetSuite release notes](https://experienceleague.corp.adobe.com/docs/sign-integrations/using/netsuite/release-notes.html?lang=en) for more information.

## Install the bundle and configure OAuth {#install}

Only a NetSuite administrator can install or update the bundle. To configure OAuth, the NetSuite administrator must have Admin access to Adobe Sign. Before installing the bundle in your Production account, you should install and test the bundle in a NetSuite Sandbox account.
...
1. On the *Search & Install Bundles* page, enter **Adobe Sign** as keyword and select **Search**.

1. Select the **Adobe Sign** bundle name.

   ![Search for the bundle](images/search-for-the-bundle.png)

1. On the *Bundle Details* page, select **Install**.
1. In the *Preview Bundle Install* page, select **Install Bundle**. 
   
   (There is no need to change any of the default values on the page)

   ![Install the bundle](images/bundle-details-install.png)

1. In the Install dialog that appears, select **OK** to proceed. 

   During the installation process, the status of the bundle displays as *Pending*.

   ![Installing bundles](images/installing-bundles.png)

1. To display an updated status, select **Refresh**. 

   After the bundle installation completes, *Adobe Sign for NetSuite* displays on the *Installed Bundles* page.
...
Adobe Sign uses OAuth 2.0 to authenticate your Adobe Sign account within NetSuite.

This protocol authorizes your installed NetSuite bundle to communicate with Adobe Sign without requesting your password. Since sensitive information is not being shared directly between the apps, your account is less likely to be compromised.
...
The NetSuite administrator who configures OAuth must also have an account level admin access to Adobe Sign.

1. In NetSuite, navigate to the *Adobe Sign Config* list page.
   
1. Search for **Adobe Sign Config** (a custom record type) using the Search field in the header.
...
1. On the Adobe Sign Config List page, select **View** for the *Using OAuth to Access Adobe Sign APIs* record.

   ![Adobe Sign configuration list](images/adobe-sign-configlist.png)

1. On the Adobe Sign Config page, select **Login In With Adobe Sign**

   ![Log in ](images/log-in-with-adobesign.png)

1. In the Adobe Sign login page that appears, enter your credentials and select **Sign In**.
 
   ![authenticate to Adobe Sign](images/8-authenticate-toadobesign-2020.png)

1. In the Confirm Access page (for OAuth) that displays, select **Allow Access**

   ![Allow Access](images/confirm-access.png)

1. When the authorization is complete, you are redirected back to Adobe Sign Config page in NetSuite, as shown below.

   ![Successful OAuth](images/successful-oauth.png)

   >[!NOTE]
   >
   >If you are configuring OAuth in your Sandbox account, you will encounter the error “Could not determine customer compid” when the authorization completes.
   >
   >
...
NetSuite bundle updates are regularly released by Adobe. Existing users of the Adobe Sign for NetSuite integration can update to the latest bundle.
...
The time required to update to the v4.0.4 bundle depends on the number of agreements that currently have an 'Out for Signature' status. Usually, it takes 7 to 10 minutes to update 100 agreements. Note the number of records to estimate your update time.

To determine the number of agreements that are out for signature:

1. Navigate to **Customization > Lists, Records, and Files > Record Types**, then locate *Adobe Sign Agreement.* 

   Or search for Adobe Sign Agreements in the search bar.

1. For the Adobe Sign Agreements record, select **Search**.

   ![Search Record Types](images/search-adobe-signagreements.png)

1. From the **Status** drop-down, select **Out for Signature** and then select **Submit**.
...
1. Navigate to **Customization > SuiteBundler > Search & Install > List** and locate your current bundle, as shown below.
...
1. From the Action drop-down menu, select **Update**.

   ![Update Action](images/update-action.png)

1. On the Preview Bundle Update page, select **Update Bundle** without changing any of the default values displayed on the page.
   ...
   >[!NOTE]
   >
   >When updating the bundle, you may get a warning message as shown below. If you have not customized your NetSuite eSignature records, you can proceed. If you are unsure, it is suggested that you install the bundle on a Sandbox account to test it first before updating the bundle in a production account.
...
1. To display an updated status, select **Refresh**.
   ...
   >[!NOTE]
   >
   >If the update appears to be taking a long time because you have a large number of *Out for Signature* agreements, you can check the **Execution Log** sub-tab for the *Adobe Sign Bundle Installation* script to determine the progress of your update. (See [Determining the progress of the update](#determineprogress)for more information.)

   After the bundle update is complete, *Adobe Sign for NetSuite* displays on the *Installed Bundles* page.
...
You can use custom preferences to specify how agreements are created and stored in NetSuite. In addition, the *Auto Provision User in Adobe Sign* preference allows you to specify whether NetSuite users are auto-provisioned in Sign services when they send agreements from NetSuite.

1. Navigate to **Setup > Company > General Preferences**.
1. Scroll down the page, then select the **Custom Preferences** sub-tab.
...
1. Enable and configure your Adobe Sign preferences as needed:
    ...
    * **Use Trans. Contact as Signer if present**: This preference is valid only if the *Use Parent Record Contact as Signer* preference is also enabled. If enabled, when generating an agreement from a Transaction Record (e.g., Quote), the primary Transaction contact defaults as the first signer. (See [Transaction Records](#transrecords)for more information.) If there is no primary Transaction contact, or if sending from NetSuite object record (e.g. Customer record, Partner record), the default recipient will be the primary contact for the customer email. The sender can easily remove or edit the default signer or add additional signers to the agreement before sending.
   ...
   **Identity Verification Methods** {#identity-verification-methods}
   ...
   Enabled identity verification method(s) can be selected when creating an agreement. If more than one identity verification method is enabled here, the Adobe Sign Agreement page displays a **Verify Signer Identity** option.
   ...
   * **Auto Provision User in Adobe Sign**: If enabled, users who send agreements in NetSuite are automatically auto-provisioned with an Adobe Sign user account.

1. Select **Save** to save your preferences.

## Configure automatic status updates {#asu}

The Adobe Sign integration bundle allows you to automatically receive updates in NetSuite regarding the status of the agreements that have been sent from NetSuite. When this feature is enabled, NetSuite always reflects the current status of your agreements. You can enable automatic status updates as follows:

1. Navigate to **Setup > Company > Enable Features.**
1. Select the **SuiteCloud** sub-tab.
...
The Adobe Sign integration bundle already exposes the Adobe Sign Agreement object with many standard NetSuite objects, including: Customer, Estimate, Lead, Opportunity, and Partner records. You can use the Adobe Sign bundle with other record types as well, including custom records.

The Agreement tab can appear with two types of NetSuite records: Entity and Transaction records. We usually assume that a Transaction record is a record (such as quote) that can be converted into a PDF document; whereas an Entity record cannot be converted to a PDF.

## Transaction records {#transrecords}

If the agreement is created from a Transaction record, the first document on the agreement record is the PDF version of the record it came from and the first recipient is the email address of the record. If you don’t want the first document to be a PDF version of the record it came from, go to **Setup > Company > General Preferences > Custom Preferences sub-tab** and disable the **Auto Attach Transaction PDF** option. See [Setting Custom Preferences](#configure) for more information.

Under Custom Preferences, you can also enable the **Use Trans. Contact as First Signer** preference if you want the primary transaction contact to be added automatically as the first signer. When associated with a Transaction record, it displays the **Agreements** and the **Send for Signature** buttons.
...
1. On the *Scripts* list page that appears, locate the script you need to deploy, then select **View**.

    * To add the *Send for Signature* button, select **Adobe Sign Estimate Button** script.
    
    * To add the *Agreements* tab, select **Adobe Sign Agreement Loader** script.

1. On the Script page, select **Deploy Script**.
...
1. On the Script Deployment page, do the following:
    ...
    * Optionally, enter script deployment ID. 
      ...
      Refer to the *Creating a Custom Script Deployment ID* topic in the NetSuite Help Center for more information. If you do not enter an ID, one is generated.
    
    * Check the **Deployed** checkbox.
    ...
    * With the **Audience** sub-tab active (active by default), select the specific roles or users that you want to grant access to. If you want to grant access to all roles and users, enable the respective **Select all** options.
    
    * Select **Save**. When the change confirmation displays, select **Go Back**.

1. select **List** at the top of the Script Deployment page to get back to the *Scripts* list page.
...
Most NetSuite roles should have permission to use Adobe Sign without additional customization. However, you may need to grant permissions for any additional custom roles that have been created.

1. Navigate to **Customization > Lists, Records, & Files > Records Types**.
...
1. On the *Record Types* page, select **Adobe Sign Agreement** to select it

   ![Agreement record types](images/search-adobe-signagreementsforedit.png)

1. On the *Custom Record Type* page, select **Use Permission List** from the *Access Type* drop-down.
...
1. Select the **Permissions** sub-tab.

   The list of roles and permissions displays.

   ![Roles and permissions](images/roles-and-permissions.png)

1. Set permissions as follows for the additional custom roles added to the “Adobe Sign Agreement” record type.
   ...
   >See the *[Setting Up a Permissions List for a Custom Record Type](https://system.netsuite.com/app/help/helpcenter.nl?fid=section_N2879931.html)* topic in the NetSuite Help Center for more information
    ...
    1. Set *Level* to **Full**
    1. Set *Default Form* to **Custom EchoSign Agreement Form**
    1. selectto check the *Restrict Form* check box
    1. select **Add** to save the changes for the role row
    ...
    * select **Save** on the *Custom Record Type* page when permissions for all roles have been set.

   The *Customer Record Type* page redisplays.
...
### Granting access to the Agreement tab and Send For Signature button  {#granting-access-to-the-agreement-tab-and-send-for-signature-button}

The Adobe Sign integration bundle already exposes the Adobe Sign Agreement object with many standard NetSuite objects (Customer, Estimate [Quote], Lead, etc.). The *Agreement* sub-tab is automatically enabled for the following types of objects: Customer, Lead, Opportunity, Partner, Prospect, Quote, and Vendor Bill.

The *Send for Signature* button is automatically enabled **only for the Quote object**.

NetSuite administrators can extend the ability to create agreements to additional CRM objects by modifying permissions to add the *Agreement* sub-tab, *Send for Signature* button, or both to those objects.
...
1. On the *Script* page, do the following:
    ...
    * selectthe **Deployments** sub-tab  
    
    * Under “*Applies to*” selectthe link for the entity that you want to modify

        * **Quote** in this example

   ![selectthe Deployments sub-tab](images/click-the-deploymentssub-tab.png)

    * selectthe **Edit** button on the *Script Deployment* page

   ![Edit script deployment](images/edit-script-deployment.png)

    * With the **Audience** sub-tab active, select the specific roles or users that you want to grant access to.

        * If you want to grant access to all roles and users, enable the respective **Select all**options
...
1. Navigate to **Customization &gt; Scripting &gt; Scripts**
1. On the *Scripts* page, locate the *Adobe Sign Agreement Loader* script (controls the *Agreements tab*)

    * select **View**

1. On the *Script* page, do the following:

    1. selectthe **Deployments** sub-tab
    1. Under “*Applies to*” selectthe link for the entity that you want to modify access for
    1. On the *Script Deployment *page, selectthe **Edit** button
    
    1. With the **Audience** sub-tab active (it is active by default), select the specific roles or users that you want to grant access to. If you want to grant access to all roles and users, enable the respective **Select all** options
    
    1. select **Save**

## Using the Adobe Sign for NetSuite bundle

In order to send agreements from NetSuite and receive updates on those agreements, users must have the same login ID (email address) in NetSuite and in Adobe Sign.
...
1. Select or create a customer record you would like to send an agreement from, or you can select another NetSuite record type that has the Agreements tab enabled.  

1. From the record, selecton the **Agreements** sub-tab.
1. select **New Agreement**.

   ![New Agreement](images/new-agreement.png)

1. On the *Adobe Sign Agreement* page, select **Edit**.
...
1. Specify the options for your agreement as follows:
    ...
    * **I Also Need to Sign This Agreement** — Enable this option to indicate that the sender also needs to sign the agreement.
    * **Signature Order**-If the *I Also Need to Sign This Agreement* option is enabled, select the order in which the sender and recipients should sign. The options are “I sign, then recipients sign”, “Recipients sign, then I sign” and “None”.
    ...
    * **Verify Signer Identity** — Enable this option then select one of the following identity verification options
        ...
        * This option display only when more than one of the three signer identity verification methods listed below is enabled in Custom Preferences. (See [Setting custom preferences](#customize) for more information.) If only one preference is enabled, the **Verify Signer Identity** option does not display.
   ...
   * **Password Required To View PDF** — Enable this option to require that a recipient enter a password before opening a PDF of the agreement or the signed agreement. The PDF file that is sent to everyone will be encrypted and this password will be required to open it. Don't lose your password as it is not recoverable. In the event that you do lose the password, you'll need to delete that transaction and begin again.
   ...
   * **Language:** Specify the language in which the signing page and email notifications will be displayed to the recipients.
...
1. On the *Documents* sub-tab, attach an existing document from the file cabinet using the *Adobe Sign Document* drop-down, and then select **Attach**.

   Or, click **New Adobe Sign Document** to access the *Adobe Sign Document* page, and then type the name of a document in your NetSuite file cabinet, select files from your Transaction record (if applicable), or attach a new document.

   You can add multiple documents to an agreement.

1. Select **Recipients** sub-tab and specify recipient by either selecting from the contact list or typing an email address.
    ...
    * **Approvers** must approve, but not sign the agreement, and may optionally need to add data to an agreement.
...
1. Select **Add** after entering each recipient.

1. Select **Save** to save the agreement.

### Send agreements for signature 

When the agreement is ready to be sent, select the **Send for Signature** button.

* If the *Preview document or position signatures* option is enabled, click **Send for Signature**. In the window that opens, preview the document or drag form fields to the document before it is sent. Select **Send** to send the agreement to the recipient.

* If the *Host Signing for First Signer* option is enabled, click **Send for Signature**. In the window that opens, allow the signer to sign the document with the sender present.
...
Adobe Sign has a direct integration with Quotes in NetSuite so that a PDF of the quote is automatically generated and attached to the agreement record.

When viewing a Quote, select **Send for Signature**. It generates and displays the quote attached to the agreement. You can also add the *Send for Signature* button to other transaction record types. See [Objects and record types](#objects)for more information.
...
* The *Send for Signature* button is replaced by the following three buttons:
   ...
   * **Update Status** —  Select this button to manually update the status if status updates have not been configured. See [Configuring automatic status updates](#asu)for more information.
   * **Send Reminder** — Select this button to send a reminder to the current signer.
   * **Cancel Agreement** — Select this button to cancel an agreement. An agreement may be canceled after it has been sent for a signature if all recipients have not yet signed.
...
To uninstall the bundle, follow the steps provided in the NetSuite Help. See the *[Uninstalling a Bundle](https://docs.oracle.com/cloud/latest/netsuitecs_gs/NSBDL/NSBDL.pdf)* topic in the NetSuite Help Center for more information.
...
Do NOT uninstall the bundle if you need to retain your unsigned agreements.
...
1. Navigate to **Customization &gt; Scripting &gt; Scripts**.
1. On the *Scripts* page, locate the *Adobe Sign Bundle Installation* script, then select **Edit**.
1. On the *Script* page, select the **Execution Log** sub-tab.
1. select **Refresh**.

   The Execution Log updates to reflect the current status. The *Details* column displays the progress of the updates to your agreements.
...
* The NetSuite/Adobe Sign administrator who configured OAuth has revoked the access token
* The access token has expired because no agreements have been sent from NetSuite in the past 60 days
* The NetSuite/Adobe Sign administrator has not successfully complete the initial OAuth configuration
...
1. Check the deployment execution log for the *Adobe Sign External Update* script to see if you are receiving calls from Adobe Sign as follows:
    ...
    1. On the *Script Deployments* page, locate the *Adobe Sign External Update* script, then select **Edit**
        1. On the *Script Deployment* page, select the **Execution Log** sub-tab.
...
1. Check the deployment execution log for the *Adobe Sign Update Agreements* script to see if there are any errors as follows:
    ...
    1. Navigate to **Customization &gt; Scripting &gt; Script Deployments**.
    1. On the *Script Deployments* page, locate the *Adobe Sign Update Agreements* script with the “Scheduled” status, then select **Edit**.
    1. On the *Script Deployment* page, select the **Execution Log** sub-tab.
    1. Under *Type*, select **Error** to filter the results.
...
1. Select **View** for the corresponding script.

1. Select the **Execution Log** sub-tab on the page to display the script log.
